...
export class TypeScriptQuestionsComponent {
  ...
  showTypeScriptQuestions(): void {
    this.http.get<Question[]>('http://localhost:5000/api/question/get-by-feild-name/typescript').subscribe(
      { next: response => this.typeScriptQuestions = response }
    );

...
export class JavaScriptQuestionsComponent implements OnDestroy {
  ...
  showJavaScriptQuestion(): void {
    this.http.get<Question[]>(this.baseApiUrl + 'question/get-by-feild-name/javascript').subscribe(
      {
        next: response => {
          if (response) {
            this.javaScriptQuestions = response;
  ...
  calcResultJavaScript(): void {
    ...
    for (var i = 0; i < question3.length; i++) {
      if (question3[i].checked == true) {
        if ((parseInt(question3[i].value)) == 2) {
          this.correctAnswers++;
          break;
        }
        else {

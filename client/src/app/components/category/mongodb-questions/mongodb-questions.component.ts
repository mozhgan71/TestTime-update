...
export class MongoDBQuestionsComponent implements OnDestroy {
  ...
  showMongoDBQuestion(): void {
    this.http.get<Question[]>(this.baseApiUrl + 'question/get-by-feild-name/mongodb').subscribe(
      {
        next: response => {
          if (response) {
            this.mongoDBQuestions = response;

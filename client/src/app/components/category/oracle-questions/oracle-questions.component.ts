...
@Component({
  ...
  styleUrls: ['./oracle-questions.component.scss'],
  imports: [RouterModule, CommonModule, MatRadioModule, MatIconModule],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class OracleQuestionsComponent implements OnDestroy {
  ...
  showOracleQuestion(): void {
    this.http.get<Question[]>(this.baseApiUrl + 'question/get-by-feild-name/oracle').subscribe(
      {
        next: response => {
          if (response) {
            this.oracleQuestions = response;
  ...
  calcResultOracle(): void {
    ...
    for (var i = 0; i < question3.length; i++) {
      if (question3[i].checked == true) {
        if ((parseInt(question3[i].value)) == 3) {
          this.correctAnswers++;
          break;
        }
        else {
    ...
    for (var i = 0; i < question10.length; i++) {
      if (question10[i].checked == true) {
        if ((parseInt(question10[i].value)) == 3) {
          this.correctAnswers++;
          break;
        }
        else {

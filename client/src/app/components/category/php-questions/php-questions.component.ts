...
export class PhpQuestionsComponent implements OnDestroy {
  ...
  showPhpQuestion(): void {
    this.http.get<Question[]>(this.baseApiUrl + 'question/get-by-feild-name/php').subscribe(
      {
        next: response => {
          if (response) {
            this.phprQuestions = response;
  ...
  calcResultPhp(): void {
    ...
    for (var i = 0; i < question3.length; i++) {
      if (question4[i].checked == true) {
        if ((parseInt(question4[i].value)) == 2) {
          this.correctAnswers++;
          break;
        }
        else {

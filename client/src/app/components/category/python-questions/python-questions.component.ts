...
export class PythonQuestionsComponent implements OnDestroy {
  ...
  showPythonQuestion(): void {
    this.http.get<Question[]>(this.baseApiUrl + 'question/get-by-feild-name/python').subscribe(
      {
        next: response => {
          if (response) {
            this.pythonQuestions = response;

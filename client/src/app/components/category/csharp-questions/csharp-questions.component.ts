...
export class CsharpQuestionsComponent implements OnDestroy {
  ...
  showCSharpQuestion(): void {
    ...
    this.http.get<Question[]>(this.baseApiUrl + 'question/get-by-feild-name/csharp').subscribe(
      {
        next: response => {
          if (response) {
            this.cSharpQuestions = response;
  ...
  calcResultCsharp(): void {
    ...
    for (var i = 0; i < question9.length; i++) {
      if (question9[i].checked == true) {
        if ((parseInt(question9[i].value)) == 3) {
          this.correctAnswers++;
          break;
        }
        else {
